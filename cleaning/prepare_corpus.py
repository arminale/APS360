...
if __name__ == "__main__":
    ...
    dets = json.load(open(r'..\data\raw\det_ingrs.json', 'r'))
    layer1 = json.load(open(r'..\data\raw\layer1.json', 'r'))
    ...
    t = time.time()
    print("Saving tokenized here:",
          r'..\data\tokenized_instructions_'+partition+'.txt')
    f = open(r'..\data\tokenized_instructions_'+partition+'.txt', 'w')
    ...
    f = open(r'..\data\cleaned\tokenized'+partition+'.txt', 'r')
    text = f.read()
    text = tok(text)
    f.close()

    f = open(r'..\data\cleaned\tokenized'+partition+'.txt', 'w')
    f.write(text)
    f.close()
